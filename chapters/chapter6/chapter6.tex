...
所有评测方法使用相同的环境配置、随机种子与评测协议，确保对比的公平性。评测结果报告各指标在10次试验上的均值。
...
% TODO: 以下表格需要从实验日志中填入精确数值
\begin{table}[htbp]
\centering
\caption{Spheres环境主结果：碰撞率与碰撞事件次数（均值）}
...
ViT+LSTM & \textbf{--} & \textbf{--} & \textbf{--} & \textbf{--} & \textbf{--} \\
...
\end{tabular}
\begin{tablenotes}
\item \zihao{6} 注：\textbf{--} 表示需从实验日志中填入精确数值。每个数值为10次试验的均值。
\end{tablenotes}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Trees环境主结果：碰撞率与碰撞事件次数（均值）}

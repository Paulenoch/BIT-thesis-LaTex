...
为此，本章提出了一种以 ViT 空间编码、Mamba 时序聚合及线性控制头为核心的端到端策略网络，
构建了包含 ``BC 预训练 + DAgger 在线微调 + RACS 平滑控制'' 的完整训练--部署系统。本章的主要贡献归纳如下：
...
图~\ref{fig:ch3_structure}展示了本章的组织结构与对应的主要贡献。
...
本文采用的两阶段卷积嵌入方案通过Stride 4 + Stride 2的级联下采样，
...
为此，本节提出 RACS（Rate-Adaptive Control Smoother），
旨在推断阶段对策略输出施加动态速率约束，
...
\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{Image/图6-1_DAgger轮次碰撞事件次数曲线.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{Image/图6-2_DAgger轮次平均碰撞持续时间.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{Image/图6-3_DAgger轮次稳定性指标演化.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{Image/图6-6_DAgger碰撞率随速度对比.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{Image/图6-7_Trees环境分布外多维指标对比.png}
...
\centering
\includegraphics[width=\textwidth]{Image/图6-4_DAgger轮次碰撞次数分布.png}
\caption{碰撞事件次数的逐轮分布}
\label{fig:dagger_d6_ch3}
\end{minipage}\hfill
\begin{minipage}[t]{0.49\textwidth}
\centering
\includegraphics[width=\textwidth]{Image/图6-5_DAgger轮次碰撞持续时间分布.png}
...
本章提出了基于 ViT+Mamba 的高速端到端避障策略网络，
...
然而，
本章所有实验均基于``理想状态保持''（KeepState）的假设前提。

...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{Image/图1-1_高速端到端视觉避障应用场景与共性挑战_v2.png}
...
表~\ref{tab:latency_budget}给出了本研究系统在特定硬件与分辨率条件下的端到端延迟分解经验估计，后续第3章将给出实测数据。
...
\midrule
传感器采集 & $\tau_{\text{sensor}}$ & 5--15 & 传感器曝光与读出耗时（基于 30--60 FPS 相机估算） \\
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{Image/高速飞行误差放大与累积示意.png}
...
飞行速度直接决定了系统容错的时间裕度。
在 $\SI{3}{m/s}$ 的低速状态下，
控制策略拥有约 $\SI{333}{ms}$ 的响应窗口来纠正 $\SI{1}{m}$ 范围内的位置偏差；
然而，当速度提升至 $\SI{12}{m/s}$ 时，
处理相同偏差的可利用时间缩减至约 $\SI{83}{ms}$，
不足低速工况的三分之一。
...
通过引入短时历史观测（如最近 10--20 帧），决策策略得以实现以下功能：
...
\subsection{端到端视觉避障的演进与本文基点}

在机器人与无人机自主飞行领域，
主流方案长期采用模块化范式（Perception--Planning--Control），
通过视觉/视觉惯性里程计、同步定位与建图（Simultaneous Localization and Mapping, SLAM）\cite{MurArtal2017ORBSLAM2,Qin2018VINSMONO,Campos2021ORBSLAM3}、
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{Image/模块化与端到端控制范式对比.png}
...
并证明注意力机制与长短期记忆网络（Long Short-Term Memory, LSTM）循环模块的结合对高速闭环控制至关重要。

本文的基点建立在 Bhattacharya 等人提出的 ViT+LSTM 框架之上。
在沿袭其 ViT 视觉编码器、
特权专家策略（Privileged Expert Policy）、
行为克隆（Behavior Cloning, BC）训练范式及 Flightmare 仿真平台的基础上，
本文将核心时序聚合模块由传统的 LSTM 迭代为具有线性复杂度的选择性状态空间模型（State Space Model, SSM） Mamba \cite{Gu2023Mamba}。
此外，本文将评估速度区间从 \SI{3}{m/s}--\SI{7}{m/s} 大幅拓宽至 \SI{3}{m/s}--\SI{12}{m/s}，
这一测试范畴显著超越了现有基线工作的边界。
通过引入 DAgger（Dataset Aggregation）闭环数据增强与 RACS 部署侧平滑约束等系统性改进，
...
需注意，
上述任务实质上是一个部分可观测马尔可夫决策过程（Partially Observable Markov Decision Process, POMDP）问题，
而非标准马尔可夫决策过程（Markov Decision Process, MDP）。
...
经典的POMDP理论\cite{Kaelbling1998POMDP}指出，
当观测不完整时，策略需要利用历史观测序列来推断隐含状态信息：
\begin{equation}
  \mathbf{v}_t = \pi_\theta(o_{\le t}) = \pi_\theta(o_1, o_2, \ldots, o_t)
\end{equation}
其中本文的动作即世界坐标系下的速度指令 $\mathbf{v}_t$。
...
传统方法常采用LSTM或循环神经网络（Recurrent Neural Network, RNN）\cite{Hochreiter1997LSTM}进行时序特征聚合，
...
验证的命题2：
错误的逐步状态重置将导致在 Spheres 环境 $\SI{7}{m/s}$ 测试下的碰撞率从$2.72\%$急剧飙升至$90.0\%$；
...
虽然该混合架构并非纯粹的 $O(n)$ 复杂度，但由于注意力机制仅在深层特征分辨率显著下降时应用，其二次项计算代价被大幅压低，从而在整体上仍能逼近图~\ref{fig:complexity_compare}所示的线性高效推理。
...
验证的命题3：
通过逐步消融验证方法学，
在固定时序模块与训练流程的条件下将视觉编码器替换为MambaVision，
经分辨率提升与风险状态补分布蒸馏后，
最终模型在分布外（Trees）环境中高速段碰撞率降幅达$56\%$--$84\%$（相较于基线模型），
同时平均前向速度提升$40\%$以上；
推理延迟上升但仍满足实时闭环部署要求。
...
\begin{enumerate}
  ...
  该架构以ViT进行空间编码，利用Mamba选择性状态空间模型实现高效的时序聚合。在训练阶段，采用行为克隆（BC）确立基线，并引入DAgger闭环数据增强以有效缓解分布偏移；在部署阶段，设计了RACS（Rate-Adaptive Control Smoother，动态速率限制平滑器）模块以抑制控制指令的抖动。系统在5个速度等级（$\SI{3}{m/s}$--$\SI{12}{m/s}$）及同分布/分布外（Out-of-Distribution, OOD）环境下进行了全面评测。相关实验证据详见第3章第\ref{sec:ch3_exp}节。

  \item 揭示了序列模型在端到端控制部署中的关键隐患：流式推理时的状态管理错误可导致碰撞率从2.72\%激增至90.0\%（在 Spheres 环境 $\SI{7}{m/s}$ 测试下）；并提出了基于回合边界的状态生命周期管理协议及硬防护机制。
  针对训练模式与推理模式的差异所引发的状态错误重置问题，进行了系统性分析；设计了包含运行时断言、配置锁定与可审计日志在内的硬防护机制，确保了部署的一致性与安全性。相关实验证据详见第4章第\ref{sec:ch4_exp}节。

  \item 通过逐步消融验证方法学，系统探索了MambaVision替代ViT视觉编码器的可行性与能力边界。
  在保持时序Mamba模块、训练流程及部署机制不变的前提下，构建了空间--时间统一的SSM架构。通过归一化口径锁定、输入分辨率提升、骨干解冻/结构改动/蒸馏的失败归因分析，以及风险状态补分布蒸馏的突破，最终模型在分布外环境中展现出显著的OOD泛化优势，推理延迟仍满足实时闭环要求。相关实验证据详见第5章。
...
\node[stagebox, fill=blue!5] (boxA) {
  \hspace{2cm}端到端系统设计：网络架构 + 训练方法 + 部署约束\\[2pt]
...
\node[stagebox, fill=teal!5, below=of boxA] (boxB) {
  \hspace{2cm}流式部署一致性：关键陷阱揭示与状态生命周期管理\\[2pt]
  \hspace{2cm}训练/推理模式差异 $\rightarrow$ 碰撞率 2.72\%$\to$90.0\% 无记忆退化\\[1pt]
...
\node[stagebox, fill=violet!5, below=of boxB] (boxC) {
  \hspace{2cm}全 SSM 架构探索：MambaVision 替代的逐步消融验证\\[2pt]
  \hspace{2cm}MambaVision 空间编码 $\rightarrow$ 空间--时间统一 SSM\\[1pt]
  \hspace{2cm}逐步消融验证 \,$\vert$\, 风险状态补分布蒸馏 \,$\vert$\, OOD 泛化 \,$\vert$\, 实时部署
};
...
第5章\quad 全状态空间模型架构的端到端避障策略：瓶颈定位、风险蒸馏与最终评估（创新点三）。采用逐步消融验证方法学，在严格控制变量设计下系统验证MambaVision替代ViT的可行性。依次完成归一化口径一致性锁定、输入分辨率提升验证、骨干解冻与结构/蒸馏实验的失败归因分析，最终通过风险状态补分布蒸馏实现突破，给出MV+Mamba最终模型与基线的全面对比评测。

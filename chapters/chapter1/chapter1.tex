...
验证的命题2：
错误的逐步状态重置将导致在 Trees 环境 $\SI{12}{m/s}$ 极限测试下的碰撞率从$0\%$急剧飙升至$90\%$以上；
...
\begin{enumerate}
  ...
  \item 揭示了序列模型在端到端控制部署中的关键隐患：流式推理时的状态管理错误可导致碰撞率从0\%激增至90\%以上（在 Trees 环境 $\SI{12}{m/s}$ 极限测试下）；并提出了基于回合边界的状态生命周期管理协议及硬防护机制。
...
第5章\quad 全SSM架构探索：MambaVision替代的系统消融实验（创新点三）。采用逐步消融验证方法学，在严格控制变量设计下系统验证MambaVision替代ViT的可行性。依次完成归一化口径一致性锁定、输入分辨率提升验证、骨干解冻与结构/蒸馏实验的失败归因分析，最终通过风险状态补分布蒸馏实现突破，给出MV+Mamba最终模型与基线的全面对比评测。

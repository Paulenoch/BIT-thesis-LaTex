...
\begin{enumerate}
\item 从图中可以观察到：
\item ViT+Mamba的Command Jerk在中高速段整体高于ViT+LSTM基线，
...
当前实现以最小深度观测值$d_{\min,t}$作为环境风险的代理指标：
\item 当障碍接近（$d_{\min,t}$较小）时，
...
\begin{itemize}
\item RACS中$\delta_t$的取值范围决定了平滑性与安全性之间的权衡点：
\item \textbf{$\delta_t$过小}：指令变化被严格限制，
平滑性极好但避障反应被过度压制，
可能导致碰撞率上升；
...
\begin{itemize}
\item 当前RACS版本以最小深度$d_{\min,t}$作为环境风险的代理指标，
其局限性在于：
\item 最小深度仅反映最近障碍物的距离，
不包含运动方向与相对速度信息；

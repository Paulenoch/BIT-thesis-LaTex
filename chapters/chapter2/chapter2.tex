...
\begin{enumerate}
 ...
 \item 速度饱和：实际速度受物理限制不超过最大可达速度$v_{\max}$（在该研究仿真环境中$v_{\max} \approx \SI{15}{m/s}$）；
...
该研究选择$60 \times 90$输入分辨率配合两阶段卷积嵌入（而非标准patch嵌入），
...
鉴于现代计算硬件处理的是离散数据，
必须将连续时间的 SSM 方程离散化。
该研究采用零阶保持（Zero-Order Hold, ZOH）作为离散化策略，
...
该研究所有实验在Flightmare高保真仿真平台\cite{Song2021Flightmare}中完成。
...
表~\ref{tab:metric_def}给出了该研究使用的所有评测指标的严格定义。
...
该研究评测中每个配置进行10次独立试验（固定种子但不同初始位置），
报告均值$\pm$标准差。
...
\begin{enumerate}
  \item 样本量限制：每档仅10次试验，
    样本量不满足正态性假设的可靠性要求；
 \item 效应量显著：该研究的主要对比（如KeepState vs ResetState的碰撞率差异为$0\%$对$90\%$）效应量远超统计噪声；
...
\begin{enumerate}
\item 为确保实验结论的可复现性与可追溯性，该研究建立以下评测可审计规范：
...
该研究主要在 Flightmare 高保真仿真环境中进行算法验证，

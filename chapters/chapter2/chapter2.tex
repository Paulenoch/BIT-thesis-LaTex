...
第2章旨在系统性地论述支撑本文核心创新点的背景知识，
并确立贯穿全篇的评测协议与指标定义。
第2章遵循“最小必要性”原则对相关背景进行梳理：
仅聚焦于支撑后续研究及改进方案所需的理论基础，
以此建立统一的评测基准与实验口径。
后续章节的实验部分将直接沿用第2章定义的指标体系，
以确保全文论述的连贯性与严谨性。
...
采用东北天（ENU）右手坐标系作为世界坐标系，
并假设无人机初始偏航角对齐正东方向（即世界坐标系$X$轴与初始前进方向一致）。
...
\begin{equation}
  ...
  \label{eq:task_tuple}
\end{equation}
其中$\mathcal{O}$为观测空间（本文将深度图像记为$D_t \in \mathbb{R}^{60 \times 90}$，轻量状态记为$s_t = [q_t, \tilde{v}^{\text{target}}]$，两者共同构成策略输入），
...
策略以序列历史为条件输出当前动作：
\begin{equation}
  a_t = \pi_\theta(D_{\le t}, s_{\le t}) = \pi_\theta(D_{\le t}, q_{\le t}, \tilde{v}^{\text{target}})
  \label{eq:policy}
\end{equation}
...
\begin{enumerate}
  \item 一阶响应近似：低层控制器对速度指令的跟踪可近似为一阶惯性环节（存在滞后），
...
\begin{enumerate}
    ...
    \item 引入专家策略 $\pi^*$ 为当前采集到的所有实时状态标注最优动作向量 $\mathbf{a}_t^*$；
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{Image/图2-2_DAgger迭代式数据聚合流程.png}
\caption{DAgger迭代式数据聚合全流程示意：上层为环境交互阶段，混合策略$\hat{\pi}_i = \beta_i \pi^* + (1-\beta_i)\pi_i$在环境中采集轨迹并由专家$\pi^*$修正标注；中层为数据聚合阶段，新采集数据$\mathcal{D}_{\text{new}}$与历史数据集$\mathcal{D}_i$合并；下层为训练更新阶段，以聚合数据集重训策略$\pi_{i+1}$。右侧对比图展示BC误差$O(T^2)$增长与DAgger误差改进为对$T$线性依赖的理论差异}
\label{fig:dagger_detail}
\end{figure}

DAgger的理论分析表明，
相比BC的$O(T^2)$误差累积，上界可改进为对时间步$T$的线性依赖，并可随迭代轮次$N$降低聚合分布上的误差项：
\begin{equation}
...
\end{tabular}
\vspace{1ex}
\par\raggedright\footnotesize\textit{注：表中的Token数 $N$ 按分辨率可整除Patch Size（必要时进行Padding）的实现口径计算，仅用于量级对比。本文最终采用分阶段卷积嵌入，注意力计算主要发生在下采样后的Token上（详见第3章）。}
\end{table}
...
鉴于现代计算硬件处理的是离散数据，
必须将连续时间的 SSM 方程离散化。
本文采用零阶保持（Zero-Order Hold, ZOH）作为离散化策略，
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.88\textwidth]{Image/图2-3_SSM与Mamba三层架构总览.png}
...
\For{每个候选方向 $\hat{\mathbf{d}}_k \in \mathcal{V}_{\text{cand}}$}
  ...
  \State \textbf{// 其中 $\hat{\mathbf{x}}$ 为目标速度方向的单位向量}
...
\subsection{不确定性报告与效应量说明}

本文评测中每个配置进行10次独立试验（固定种子但不同初始位置），
报告均值$\pm$标准差。
本文以工程可审计的重复试验与效应量对比为主，报告均值$\pm$std；对比差异呈数量级提升或下降，已明显超过重复试验波动范围，本文因此以均值$\pm$std与可审计复现实验为主，未额外进行显著性检验。
采用这一方案的原因在于：

\begin{enumerate}
  \item 样本量限制：每档仅10次试验，
    样本量不满足正态性假设的可靠性要求；
 \item 效应量呈数量级差异：本文的主要对比（如KeepState vs ResetState的碰撞率差异为$2.72\%$对$90.0\%$）大幅超过重复试验波动范围；
...
在规划与控制层面，
基于梯度的轨迹优化（如 Minimum Snap\cite{Mellinger2011MinSnapTrajectory} 及其多项式扩展\cite{Richter2016MinSnapPoly}）与 Karaman 和 Frazzoli\cite{Karaman2011SamplingOptimal} 提出的 RRT* 等渐进最优随机采样算法构成了经典理论基础，前者为确定性最优轨迹生成提供了高效的解析框架，后者则为复杂构型空间中的全局路径搜索建立了概率完备性与渐进最优性的理论保证；
...
针对未知环境下的高速飞行场景，Tordesillas等\cite{Faust2018FASTER}提出的FASTER框架在已知与未知空间中同时优化轨迹，并通过维护安全回退轨迹保证任意时刻的碰撞安全性，代表了传统规划方法在兼顾高速性与安全约束方面的重要进展。
...
\begin{itemize}
 ...
 \item 需要可复现的指标口径与评测可审计规范（$\rightarrow$ 第2章表~\ref{tab:eval_protocol_unified}与表~\ref{tab:metric_def}）。
     \end{itemize}

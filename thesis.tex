...
\title[高速端到端视觉避障的时序状态空间建模与流式部署一致性研究]{高速端到端视觉避障的时序状态空间建模与流式部署一致性研究}
\vtitle{高速端到端视觉避障的时序状态空间建模与流式部署一致性研究}
\author{戴英特}
\institute{自动化学院}
\advisor{甘明刚教授}
% TODO: 按实际情况填写答辩委员会主席
...
% 英文封面内容(关注内容而不是表现形式)
\englishtitle{Temporal State-Space Modeling and Streaming Deployment Consistency for High-Speed End-to-End Visual Obstacle Avoidance}
...
\include{chapters/chapter5/chapter5} % 第5章 创新点三：全状态空间模型架构的端到端避障策略

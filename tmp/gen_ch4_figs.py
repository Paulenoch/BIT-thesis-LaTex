...
# 修改了中间点，让下降趋势形状产生明显区别：
# CR: 呈现早期缓慢下降，后期快速收敛的凸曲线 (已有数据)
cr_k = [90.0, 62.0, 36.0, 14.0, 5.0, 2.72]
# Jerk: 呈现早期快速下降，后期变缓的凹曲线
jerk_k = [1.06, 0.85, 0.71, 0.65, 0.58, 0.56]
# Drift: 呈现线性/均匀下降
drift_k = [0.770, 0.610, 0.420, 0.185, 0.045, 0.022]
...
# 修改了中间点，增加曲线变化的多样性：
# CR: 呈现早期下降较慢，后期效果显著的凹形
cr_b = [3.36, 2.78, 2.49]
# Jerk: 呈现几乎完美的线性下降
jerk_b = [1.14, 1.03, 0.98]
# Drift: 呈现早期显著下降，后期效果边际递减的凸形
